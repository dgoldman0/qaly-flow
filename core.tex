...
Assumption~\ref{as:m} (a uniform lower bound $m>0$ on expected cycle length) holds if there is a uniform \emph{upper} bound on catastrophe rates, preventing cycles from ending arbitrarily quickly. Formally: if $\sup_{x\in\cS,a\in\cA(x)} Q(\absorb\mid x,a) \le M < \infty$, then the process cannot reach $\absorb$ faster than rate $M$, giving $\E[\tau^\pi] \ge 1/M > 0$. Alternatively, explicit modeling of minimum repair or decision times (a positive lower bound on sojourn durations in non-absorbing states) directly ensures $\E[\tau^\pi] \ge m > 0$. For Assumption~\ref{as:lipschitz}, if transition rates $Q_\theta(y\mid x,a)$ are Lipschitz in $\theta$ uniformly over $x,a,y$, then standard perturbation bounds for Markov chain expectations (via Dynkin's formula or coupling arguments) yield Lipschitz continuity of $\E_\theta[R^\pi]$ and $\E_\theta[\tau^\pi]$.
\end{remark}
...
\section{Relation to the broader ensemble}

\textbf{Ensemble ID:} \texttt{c2f3de53-f086}

This paper defines the QALY flow framework as a mathematical foundation for evaluating health policy when biological lifespan has no fixed upper bound. It provides the formal model---regenerative processes, renewal-reward objectives, cost-effectiveness ratios, perturbation stability, and distributional aggregation---while remaining deliberately agnostic about the broader civilizational, translational, and engineering contexts in which the framework would be deployed.

Three companion papers extend this foundation:

\begin{itemize}
  \item \textbf{Longevity Civilizations: Life, Craft, and Depth After the Defeat of Senescence} (\texttt{73997063-e1ed}) explores what societies look like once senescence has been largely defeated: how institutions, identity, measurement, and safety culture reshape around open-ended horizons. It grounds the framework in the lived texture of a longevity civilization and shows how the sustained-present perspective emerges naturally from the conditions of indefinite life.

  \item \textbf{Sustaining Health Over Long Horizons: Integrated Care, Maintenance Burden, and Equity When Ageing Becomes Manageable} (\texttt{80fbe599-356c}) translates the QALY flow framework into public health practice. It applies the rate-based perspective to integrated care delivery, maintenance burden assessment, and equity in sustained access, drawing on WHO implementation guidance and existing health systems infrastructure.

  \item \textbf{Future-Building as Maintenance: Rate-Based Health Evaluation for Indefinite Horizons} (\texttt{d7006442-c67e}) connects aging biology, reliability engineering, existential-risk governance, and institutional measurement design under a unified maintenance framing. It shows how the rate-based perspective unifies concerns across biological, social, and civilizational scales without requiring the formal apparatus developed here.
\end{itemize}

Together, the ensemble provides: (1)~a rigorous mathematical foundation (this paper), (2)~civilizational vision and context, (3)~public health translation into existing delivery and equity frameworks, and (4)~cross-scale synthesis connecting engineering, governance, and measurement. Each paper is self-contained but gains context from the others.
